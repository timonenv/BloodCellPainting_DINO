...
from vision_transformer_BN import DINOHead 
import utils
...
def get_args_parser():
    ...
    # Misc
    parser.add_argument('--gradient_accumulation_steps', type=int, default=16, help="""How many steps for gradient accumulation""")
    parser.add_argument('--dataset_dir', default=".", type=str, help='Path for dataset.') 
    ...
    parser.add_argument('--train_datasetsplit_fraction', default=1.0, type=float, help='fraction of training dataset to use for training')
...
def train_dino(args, save_dir):
    ...
    # calculated previously for each channel
    mean_for_selected_channel, std_for_selected_channel = (0.004043382126837969, 0.009857425466179848, 0.007302745711058378, 0.009263753890991211), (0.004476326052099466, 0.01167044322937727, 0.0035245809704065323, 0.011316084302961826) # TODO without brightfield
    ...
    class Multichannel_dataset(torch.utils.data.Dataset):
        def __init__(self, root_path, transform=None):
            self.data_paths = glob.glob(root_path + '/***/**/*.npy', recursive=True) # TODO was png, /**/*.npy'
            self.transform = transform
            self.root_path = root_path
        def __getitem__(self, idx):
            path = self.data_paths[idx] # index is always in the same order for each folder
            image_np = np.load(path).astype(np.float32)
            image_np = image_np / 65535.0 # images are 16bit
            image = torch.from_numpy(image_np).float() # float 32
    ...
    print(f"Data loaded: there are {len(dataset_total)} images.")
    sampler = torch.utils.data.DistributedSampler(dataset_total, shuffle=True) # TODO added

    data_loader = torch.utils.data.DataLoader(
        dataset_total,
        sampler=sampler, #train_sampler_wrapped
...
def train_one_epoch(student, teacher, teacher_without_ddp, dino_loss, data_loader,
    ...
    for it, images in enumerate(metric_logger.log_every(data_loader, 10, header)):
        ...
        metric_logger.update(loss=unscaled_loss.item()) # TODO was metric_logger.update(loss=loss.item())
        metric_logger.update(lr=optimizer.param_groups[0]["lr"])
        metric_logger.update(wd=optimizer.param_groups[0]["weight_decay"])
        
        if (it+1) % gradient_accumulation_steps == 0:
            wandb.log({"loss": unscaled_loss})  #wandb.log({"loss": loss}) 
...
class DataAugmentationDINO(object):
    def __init__(self,global_crops_scale, local_crops_scale, local_crops_number, mean_for_selected_channel,std_for_selected_channel):
        ...
        self.global_transfo1 = transforms.Compose([ 
            transforms.RandomResizedCrop(224, scale=global_crops_scale, interpolation=transforms.InterpolationMode.NEAREST),
        ...
        self.local_transfo = transforms.Compose([
            transforms.RandomResizedCrop(96, scale=local_crops_scale, interpolation=transforms.InterpolationMode.NEAREST),
            flip_gamma_brightness,
...
if __name__ == '__main__':
    ...
    train_dino(args, args.output_dir)